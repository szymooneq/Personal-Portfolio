      ...
      <Article>
        ...
        {projectData.stack && (
          <Group type="list" title="Stack:">
            {projectData.stack.map((el) => <li key={el.title}><a href={el.url} className={styles.stackUrl} target="_blank" rel="noreferrer">{el.title}</a> - {el.description}</li>)}
          </Group>
        )}
        ...
        <Group type="links">
          {projectData.links.map(({ link, url }) => (
            <a key={url} href={url} className="button" target="_blank" rel="noreferrer">
              {link.alt}<Image src={urlFor(link.icon).url()} alt={link.alt} width={20} height={20} />
            </a>
          ))}
        </Group>
